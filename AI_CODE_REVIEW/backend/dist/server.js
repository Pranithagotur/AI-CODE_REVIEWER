...
app.post('/api/auth/signup', async (req, res) => {
    ...
    try {
        ...
        const dbResult = await (0, db_1.query)('INSERT INTO users (name, email, password) VALUES (?, ?, ?)', [name, email, hashedPassword]);
        res.status(201).json({
...
// --- CODE REVIEW ROUTE ---
app.post('/api/reviews/analyze', optionalAuthenticateToken, async (req, res) => {
    ...
    try {
        ...
        // 1. Run Static Rules
        const ruleFeedback = (0, ruleEngine_1.runStaticRules)(language, sourceCode);
        // 2. Run Execution Sandbox
        const runtimeFeedback = await (0, sandbox_1.executeCode)(language, sourceCode);
        // 3. Request AI Review Feedback
        const aiFeedback = await (0, aiReviewer_1.getAiCodeReview)(language, sourceCode, selectedMode);
        // 4. Save review results in DB if authenticated
        let reviewId = null;
        if (req.user) {
            const dbResult = await (0, db_1.query)('INSERT INTO code_reviews (user_id, language, source_code, mode, ai_feedback, rule_feedback, runtime_feedback) VALUES (?, ?, ?, ?, ?, ?, ?)', [
                req.user.id,
...
app.get('/api/reviews/history', authenticateToken, async (req, res) => {
    try {
        const rows = await (0, db_1.query)('SELECT id, language, mode, created_at FROM code_reviews WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
        res.json(rows);
    }
    catch (err) {
...
app.get('/api/reviews/:id', authenticateToken, async (req, res) => {
    ...
    try {
        const rows = await (0, db_1.query)('SELECT * FROM code_reviews WHERE id = ? AND user_id = ?', [reviewId, req.user.id]);
        if (rows.length === 0) {
...
app.get('/api/kb/search', async (req, res) => {
    ...
    if (category || language) {
        ...
        if (language) {
            ...
            params.push(language.toLowerCase(), 'all');
        }
...
app.post('/api/kb/add', optionalAuthenticateToken, async (req, res) => {
    ...
    try {
        const dbResult = await (0, db_1.query)('INSERT INTO knowledge_base (title, category, language, pattern, solution) VALUES (?, ?, ?, ?, ?)', [title, category, language.toLowerCase(), pattern, solution]);
...
app.get('/api/analytics/summary', authenticateToken, async (req, res) => {
    try {
        ...
        const totalReviewsRow = await (0, db_1.query)('SELECT COUNT(*) as count FROM code_reviews WHERE user_id = ?', [userId]);
        const totalReviews = totalReviewsRow[0]?.count || 0;
        // 2. Language distribution
        const languageRows = await (0, db_1.query)('SELECT language, COUNT(*) as count FROM code_reviews WHERE user_id = ? GROUP BY language', [userId]);
        // 3. Recent quality trends
        const recentReviews = await (0, db_1.query)('SELECT id, ai_feedback, created_at FROM code_reviews WHERE user_id = ? ORDER BY created_at DESC LIMIT 10', [userId]);
        // Analyze common mistakes from rule feedbacks
        const allReviews = await (0, db_1.query)('SELECT rule_feedback FROM code_reviews WHERE user_id = ?', [userId]);
        ...
        const gradeHistory = recentReviews.map((r) => {
            let grade = 'A';
            try {
