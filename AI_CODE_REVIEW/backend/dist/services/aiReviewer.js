...
async function getAiCodeReview(language, sourceCode, mode = 'developer') {
...
function buildReviewPrompt(language, code, mode) {
...
async function simulateOfflineAiReview(language, code, mode) {
    const staticViolations = (0, ruleEngine_1.runStaticRules)(language, code);
    const isStudent = mode === 'student';
    ...
    let articles = [];
    try {
        articles = await (0, db_1.query)('SELECT title, category, pattern, solution FROM knowledge_base WHERE language = ? OR language = ?', [language.toLowerCase(), 'all']);
    }
    catch (dbErr) {
    ...
    articles.forEach(art => {
        ...
        const keywords = art.pattern.split(/\s+/).filter((k) => k.length > 3);
        const matchesKeyword = keywords.length > 0 && keywords.every((kw) => code.toLowerCase().includes(kw.toLowerCase()));
        if (matchesKeyword) {
    ...
    const response = {
        ...
        code_quality: {
            rating,
            summary: `[OFFLINE ANALYSIS] Reviewed ${language} code. Detected ${errors} critical errors and ${warnings} warnings. ${isStudent
