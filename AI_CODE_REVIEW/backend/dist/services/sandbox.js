...
const TEMP_DIR = path_1.default.resolve(__dirname, '../../temp_sandbox');
...
function verifyCodeSafety(lang, code) {
...
function createTempFiles(lang, id, code) {
    const runDir = path_1.default.join(TEMP_DIR, id);
    fs_1.default.mkdirSync(runDir);
    if (lang === 'javascript' || lang === 'js') {
        const file = path_1.default.join(runDir, 'index.js');
        fs_1.default.writeFileSync(file, code);
        return { dir: runDir, sourceFile: file, sourceBaseName: 'index.js' };
    }
    if (lang === 'python' || lang === 'py') {
        const file = path_1.default.join(runDir, 'main.py');
        fs_1.default.writeFileSync(file, code);
        return { dir: runDir, sourceFile: file, sourceBaseName: 'main.py' };
    }
    if (lang === 'cpp' || lang === 'c++') {
        const file = path_1.default.join(runDir, 'main.cpp');
        const outFile = path_1.default.join(runDir, 'main.exe');
        fs_1.default.writeFileSync(file, code);
    ...
    if (lang === 'java') {
        ...
        const file = path_1.default.join(runDir, `${className}.java`);
        fs_1.default.writeFileSync(file, code);
...
function cleanupTempFiles(paths) {
    try {
        if (fs_1.default.existsSync(paths.dir)) {
            // delete files recursively
            fs_1.default.rmSync(paths.dir, { recursive: true, force: true });
        }
    }
    catch (err) {
...
function runInDocker(lang, paths, startTime) {
    return new Promise((resolve) => {
        ...
        if (lang === 'javascript' || lang === 'js') {
            ...
            (0, child_process_1.exec)(compileCmd, { timeout: 5000 }, (cErr, cStdout, cStderr) => {
                if (cErr || cStderr) {
            ...
            (0, child_process_1.exec)(compileCmd, { timeout: 6000 }, (cErr, cStdout, cStderr) => {
                if (cErr || cStderr) {
...
function runLocalProcess(lang, paths, startTime) {
    return new Promise((resolve) => {
        let compileCmd = '';
        let runCmd = '';
        if (lang === 'javascript' || lang === 'js') {
...
function executeRunCommand(cmd, lang, startTime, resolve) {
    (0, child_process_1.exec)(cmd, { timeout: 2000, maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
...
function parseCompilerError(lang, output) {
    let line;
    let fix;
    if (lang === 'cpp' || lang === 'c++') {
...
function parseRuntimeException(lang, output) {
    let line;
    let fix;
    if (lang === 'python' || lang === 'py') {
...
function generateCompilerFix(lang, message) {
...
function generateRuntimeFix(lang, errType, errMsg) {
