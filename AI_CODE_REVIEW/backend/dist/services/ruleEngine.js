...
function runStaticRules(language, sourceCode) {
    const violations = [];
...
function checkHardcodedSecrets(lines, violations) {
...
function checkDuplicateCode(lines, violations) {
    const minLines = 3;
...
// --- Python Rules ---
function checkPythonRules(lines, fullText, violations) {
    ...
    lines.forEach((lineText, idx) => {
        ...
        while ((match = localDeclRegex.exec(lineText)) !== null) {
            ...
            if (varName === 'self' || varName.toUpperCase() === varName)
                continue;
            declaredVars.add(varName);
            varLineMap.set(varName, lineNum);
...
// --- C++ Rules ---
function checkCppRules(lines, fullText, violations) {
...
// --- Java Rules ---
function checkJavaRules(lines, fullText, violations) {
    lines.forEach((lineText, idx) => {
        const lineNum = idx + 1;
        if (isCommentLine(lineText))
            return;
...
function hasLoopBreaker(lines, loopStartIdx, commentChar = '//') {
    let openBraces = 0;
...
function isInsideTryCatch(lines, currentIdx) {
    let depth = 0;
...
function isInsideTryExcept(lines, currentIdx) {
